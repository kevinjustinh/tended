...
final class TaskViewModel {
    ...
    func saveTask(in context: ModelContext) {
        if let existing = editingTask {
            applyForm(to: existing, pet: formPets.first)
            NotificationService.shared.cancelReminder(for: existing)
            NotificationService.shared.scheduleReminder(for: existing)
            // Create new tasks for any additional pets selected during edit
            for pet in formPets.dropFirst() {
                let task = TendedTask()
                applyForm(to: task, pet: pet)
                if task.isRecurring {
            ...
            // Create one task per selected pet (or one pet-less task if none selected)
            let targetPets: [Pet?] = formPets.isEmpty ? [nil] : formPets.map { Optional($0) }
    ...
    func tasksByCategory(_ tasks: [TendedTask]) -> [(TaskCategory, [TendedTask])] {
        ...
        return TaskCategory.allCases.compactMap { cat in
            ...
            return (cat, list.sorted {
                ...
                return ($0.dueTimeSeconds ?? 0) < ($1.dueTimeSeconds ?? 0)
            })
        }
    }
    ...
    private func applyForm(to task: TendedTask, pet: Pet?) {
