        ...
        NavigationStack {
                        ...
                        VStack(alignment: .leading, spacing: Spacing.sm) {
                            ...
                            ScrollView(.horizontal, showsIndicators: false) {
                                HStack(spacing: Spacing.sm) {
                                    ForEach(pets) { pet in
                                        Button {
                                            taskVM.formPet = taskVM.formPet?.id == pet.id ? nil : pet
                                        } label: {
                                            HStack(spacing: Spacing.xs) {
                                                PetAvatarView(pet: pet, size: 22)
                                                Text(pet.name)
                                                    .font(.cardTitle(size: 13))
                                                    .foregroundStyle(taskVM.formPet?.id == pet.id ? .white : Color.textPrimary)
                                            }
                                            .padding(.horizontal, Spacing.md)
                                            .padding(.vertical, Spacing.sm)
                                            .background(
                                                taskVM.formPet?.id == pet.id ? Color.sageGreen : Color.softLinen,
                                                in: Capsule()
                                            )
                                        }
                                    }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(!taskVM.isFormValid)
                    .fontWeight(.semibold)
                    .foregroundStyle(taskVM.isFormValid ? Color.sageGreen : Color.textSecondary)
                }
            }
        }
